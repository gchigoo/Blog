...
test('detailed collector keeps a parser-error human in human metrics and detail collection', () => {
  const db = new Database(':memory:');
  db.pragma('foreign_keys = ON');
  initializeAnalytics(db);
  let finish;
  ...
  const middleware = createAnalyticsMiddleware({
    ...
    clientParser: { parse: () => ({ status: 'error', data: null, errorCategory: 'parse_failed' }) },
    tokenSigner: {
      createEventId: () => '11111111111111111111111111111111',
  ...
  assert.equal(response.locals.analyticsEventToken, 'v1.human-parser-error.signature');
  finish();

  assert.deepEqual(db.prepare(`
    SELECT m.traffic_kind AS metric_kind, d.traffic_kind AS detail_kind,
           d.bot_name, d.client_parse_status
    FROM access_metrics m JOIN access_event_details d ON d.metric_id = m.id
  `).get(), {
    metric_kind: 'human', detail_kind: 'human', bot_name: null, client_parse_status: 'error'
  });
  db.close();
});
